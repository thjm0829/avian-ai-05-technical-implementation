...
본 사업의 기술성은 아직 확보하지 않은 진단성능 수치가 아니라, **구현 가능한 모듈을 명확히 분리하고 각 모듈의 실패조건과 검증방법을 제시한다는 점**으로 입증한다. 카메라·MCU·모터·센서·데이터베이스처럼 현재 구현 가능한 기술과, 실제 분변·상용 키트·기준검사 비교처럼 전문기관 검증이 필요한 기술을 구분한다.

| 기술모듈 | 현재 구현근거 | 공모전 이후 시제품 산출물 | 검증방법 |
|---|---|---|---|
| Sampling Unit | 계분벨트 낙하구간, Swing/Pivot Cup과 센서·액추에이터 개념설계 | 벨트 모형과 반복 소량채취 장치 | 운전방해, 복귀, 채취 성공률과 반복편차 측정 |
| Composite Cartridge·Wet Zone | 일회용 검체접촉부, 혼합·침전·상층액 흐름 설계 | 밀폐형 전처리 카트리지와 분주 메커니즘 | 누액, Carry-over, 혼합·분주 반복성 시험 |
| Camera Reader | 고정 조명·거리, ROI와 C선·T선 영상처리 구조 | Mock 카세트 촬영·판독 Prototype | 기준판독 일치, 반복촬영 변동, Invalid·QC 시험 |
| AI Early Warning Engine | 규칙기반 재검, 변화량·이동평균·연속상승 특징값 정의 | Mock 시계열과 Reason Code Dashboard | 정상·이상·무효 시나리오 재현과 설명가능성 확인 |
| 통합 플랫폼 | Farm–Device–Sample–Test–Alert 데이터 구조 | 장치부터 경보·조치이력까지 연결된 Demo | 데이터 추적성, 장애복구, 사용자별 화면흐름 확인 |
| 전문기관 비교검증 | 기준검사와의 Paired Result 시험계획 | 승인된 시험계획과 실증 데이터셋 | 민감도·특이도·일치도·선행시간은 전문기관에서 평가 |

### AI 적용의 필요성과 범위

- 영상 AI는 사람이 육안으로 판독하던 C선·T선을 일정한 촬영조건에서 디지털화하고 QC 정보를 남기는 데 사용한다.
- 시계열 AI는 단일 양성·음성보다 반복검사의 변화방향, 연속 이상과 재검 결과를 분석해 확인 우선순위와 Reason Code를 제시한다.
- 공식검사 Label이 부족한 초기에는 Rule-Based와 통계적 이상탐지를 사용하고, 감염 여부를 예측·확정하는 머신러닝은 적용하지 않는다.
- 충분한 Paired Result가 확보된 뒤에만 모델 후보를 비교하며 정확도뿐 아니라 재현성·설명가능성·경보피로를 함께 평가한다.

| AI 단계 | 적용범위 | 현재 상태 | 확정에 필요한 근거 |
|---|---|---|---|
| 1단계 | 고정광학 C/T 판독, 이미지 QC | OpenCV·규칙기반으로 구현 | Mock 카세트 반복촬영·기준판독 비교 |
| 2단계 | 변화량·이동평균·연속이상·자동재검 | 규칙·통계 이상탐지 설계 | 정상농장 시계열과 경보피로 평가 |
| 3단계 | 공식 Label 기반 ML 위험예측 | 후속기술 | 충분한 Paired Result, 외부검증, 모델 변경관리 |
| 장기 | Movement/Network Risk | 현재 MVP 제외 | 기관협력, 법적 권한, 데이터 품질 |

### 핵심 가설이 성립하지 않을 때의 축소안

| 실패 또는 한계 | 제품범위 조정 |
|---|---|
| 단일 Sampling Point의 대표성이 부족함 | 축사 전체 대표성을 주장하지 않고 지점별 감시로 한정하며 Multi-point 비교실증 후 확장 |
| 분변 전처리가 상용 키트 IFU와 맞지 않음 | 키트·전처리 구조를 재선정하고 자동분주 범위를 축소한 판독·기록 보조형으로 조정 |
| T/C 신호 반복성이 부족함 | 반정량 추세기능을 제외하고 정성판독·이미지 QC·재검·추적성 중심으로 운영 |
| 반복추세가 공식검사 연결을 앞당기지 못함 | 조기예측 주장을 철회하고 자동 검사기록·검사누락 방지·재검 우선순위 지원으로 가치범위 축소 |
| 현장 유지관리 부담이 큼 | 일회용 Wet Zone, 모듈교환과 중앙 유지관리 구조를 재설계하고 도입대상을 좁힘 |

이 축소안은 실패를 감추기 위한 대안이 아니라, 실증결과에 따라 제품의 사용목적과 주장을 안전하게 조정하기 위한 개발 Gate다.
